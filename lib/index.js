...
const onRefresh = () => {
  document.querySelector(".Sent").classList.add("hidden");
  ul.innerText = '';
  fetch(baseUrl)
...
const cycle = () => {
  num -= 1;
   if (num === 1) {
    document.querySelector("#countdown").innerText = `${num} second`;
  }
   if (num > 1) {
    document.querySelector("#countdown").innerText = `${num} seconds`;
  }
  document.querySelector("#countdown").innerText = `${num} seconds`;
  if (num === 0) {
